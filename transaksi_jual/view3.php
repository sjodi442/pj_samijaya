...
<hr>
  ...
  while ($tampil = mysqli_fetch_array($query)){
    ...
    $query1=mysqli_query($konek, $select1);
    $tampil1 = mysqli_fetch_array($query1);
    $select3="select nama_produk from produk where id_produk='$tampil[id_produk]'";
    $query3=mysqli_query($konek, $select3);
    $tampil3 = mysqli_fetch_array($query3);
    $a = $tampil['harga'] ; $b = $tampil['jumlah'];
    $total = $a*$b;
    $tot_harga+=$total;
    $i++;
    $arr = array($i => $tampil['id_produk']);
  echo "
		<tr>
			<td align='center'>$tampil1[nama_customer]</td>
			<td align='center'>$tampil3[nama_produk]</td>";
?>
