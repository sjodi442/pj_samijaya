...
if(!empty($tampil['harga'])){
  $select="select transaksi_kirim.id_customer, transaksi_kirim.id_produk, kebijakan.harga, transaksi_kirim.jumlah from transaksi_kirim inner join kebijakan on transaksi_kirim.id_customer=kebijakan.id_customer and transaksi_kirim.id_produk=kebijakan.id_produk where transaksi_kirim.no_kirim='$sqlidp'";
  $select00 = "select transaksi_kirim.id_customer, transaksi_kirim.id_produk, produk.harga, transaksi_kirim.jumlah  from transaksi_kirim inner join customer on transaksi_kirim.id_customer=customer.id_customer inner join produk on transaksi_kirim.id_produk=produk.id_produk where transaksi_kirim.no_kirim='$sqlidp'";
  $query00 = mysqli_query($konek, $select00);
  $row = mysqli_num_rows($query00); $limit = $row - 1;
  $select0="select transaksi_kirim.id_customer, transaksi_kirim.id_produk, produk.harga, transaksi_kirim.jumlah  from transaksi_kirim inner join customer on transaksi_kirim.id_customer=customer.id_customer inner join produk on transaksi_kirim.id_produk=produk.id_produk where transaksi_kirim.no_kirim='$sqlidp' order by id DESC LIMIT $limit";
  $query0=mysqli_query($konek, $select0);
  $query=mysqli_query($konek, $select);
  $tot_harga=0;
